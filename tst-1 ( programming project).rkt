...
(define remove-selected-from-top-bar-button #f) ; NEW: Placeholder for button in top bar
...
(define fan-panel (new vertical-panel% [parent main-panel] [alignment '(left top)] [stretchable-height #t] [stretchable-width #t]))
(define fan-notebook (new tab-panel% [parent fan-panel] [choices '("Search Concerts" "Selected Concerts")] [stretchable-height #t] [stretchable-width #t]))

;; Fan - Search Concerts Tab
;; Corrected: search-content-wrapper is now a group-box-panel% for the framed look
(define search-panel (new vertical-panel% [parent fan-notebook] [alignment '(center center)] [stretchable-height #t] [stretchable-width #t]))

;; NEW: This acts as the framed container for the search form and results
(define search-content-wrapper
  (new group-box-panel%
       [parent search-panel]
       [label "Concert Search & Results"] ; Added a label for the group box
       [stretchable-height #t]
       [stretchable-width #f] ; Do not stretch horizontally, allows centering
       [min-width 400]))

;; NEW: This inner panel holds the form and results, and manages its own vertical spacing
(define search-inner-content-panel
  (new vertical-panel%
       [parent search-content-wrapper]
       [stretchable-height #t]
       [stretchable-width #t]
       ;; Removed [vert-gap 10] from here as it was causing the error
       ))

(define search-form-panel (new vertical-panel% [parent search-inner-content-panel] [stretchable-width #t] [stretchable-height #f])) ; Removed vert-gap here as well
...
(define show-all-concerts-button #f) ; Placeholder
...
;; Fan - Selected Concerts Tab
(define selected-panel (new vertical-panel% [parent fan-notebook] [min-width 600] [min-height 400] [stretchable-width #t] [stretchable-height #t]))
(define selected-concerts-list (new list-box% [parent selected-panel] [label "My Selected Concerts:"] [choices '()] [stretchable-width #t] [stretchable-height #t] [style '(single)]))
  ...
  (if (string=? (user-type current-user) "Band")
      ...
      (begin
        ...
        (unless search-results-display-panel
          (set! search-results-display-panel (new vertical-panel% [parent search-inner-content-panel] ; Parent is now search-inner-content-panel
                                                         [alignment '(left top)]
                                                         [stretchable-width #t]
                                                         [stretchable-height #t]
                                                         [style '(auto-vscroll)])))
  ...
  (let ((bn (send search-band-field get-value))
    ...
    (if (and (string=? bn "") (string=? d "") (string=? g ""))
        ...
        (let ((results (search-concerts bn d g)))
          ...
          (refresh-search-results)
          (send search-message set-label "") ; Clear previous message
          (when (empty? results)
...
(define (refresh-search-results)
  ;; FIX: Correctly delete children by iterating and calling delete on each
  ;; Only delete children of search-results-display-panel, not the whole wrapper
  ...
  (send search-results-display-panel scroll-to-show 0 0)) ; Scroll to top
...
(define (make-concert-result-panel concert-obj parent-panel)
  (let* ((h-panel (new horizontal-panel% [parent parent-panel]
                                         ...
                                         [vert-gap 5] ; Spacing between result items
                                         [horiz-margin 2]
                                         [vert-margin 2]))
...
(define (handle-star-concert concert-obj)
  ...
  (send fan-notebook set-selection 1) ; Switch to "Selected Concerts" tab
  (sleep 1) ; Short delay
...
(define (show-edit-dialog concert)
  ...
  (set! edit-concert-id (concert-id concert)) ; Store ID of concert being edited
...
;; NEW: Top-bar "Remove from Selected" button
(set! remove-selected-from-top-bar-button
  (new button% [parent top-bar-panel]
               ...
               [horiz-margin 5]))
;; Explicitly hide it after creation to avoid the 'visible #f' unused argument error
(send remove-selected-from-top-bar-button show #f)
